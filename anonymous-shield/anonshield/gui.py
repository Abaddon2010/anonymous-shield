...
class MainWindow(QMainWindow):
    ...
    def _page_home(self) -> QWidget:
        ...
        for mode in ("rapido", "anonimo", "censura", "furtivo"):
            b = _mkbtn(mode, "seg")
    ...
    def _page_bridges(self) -> QWidget:
        ...
        altrow = QHBoxLayout()
        self.btn_br_mail = _mkbtn("")
        self.btn_br_mail.clicked.connect(
            lambda: QDesktopServices.openUrl(QUrl("mailto:bridges@torproject.org?subject=get%20transport%20obfs4&body=get%20transport%20obfs4", QUrl.ParsingMode.TolerantMode)))
        self.btn_br_tg = _mkbtn("")
        self.btn_br_tg.clicked.connect(lambda: QDesktopServices.openUrl(QUrl("https://t.me/GetBridgesBot")))
        altrow.addWidget(self.btn_br_mail)
        altrow.addWidget(self.btn_br_tg)
        cl.addLayout(altrow)
    ...
    def _page_diag(self) -> QWidget:
        ...
        netrow.setSpacing(8)
        self.net_cards = {}
        for _cid in ("dns", "tcp", "http"):
            _fr = QFrame()
            _fr.setObjectName("inner")
            _vl = QVBoxLayout(_fr)
            _vl.setContentsMargins(10, 8, 10, 8)
            _vl.setSpacing(1)
            _t = QLabel()
            _t.setObjectName("kpiTitle")
            _v = QLabel("—")
            _v.setObjectName("kpiValue")
            _v.setStyleSheet("font-size: 15px;")
            _s = QLabel("")
            _s.setObjectName("kpiSub")
            _vl.addWidget(_t)
            _vl.addWidget(_v)
            _vl.addWidget(_s)
            netrow.addWidget(_fr, 1)
            self.net_cards[_cid] = (_t, _v, _s)
        nl.addLayout(netrow)
    ...
    def _page_update(self) -> QWidget:
        ...
        self.edit_repo.setReadOnly(True)
